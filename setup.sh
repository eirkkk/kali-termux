...
if [ $version_choice -eq 1 ]; then
   ...
   wget -q https://kali.download/nethunter-images/current/rootfs/kalifs-arm64-minimal.tar.xz > /dev/null 2>&1
   sudo tar -xf kalifs-arm64-minimal.tar.xz -C /data/local/
   sudo mkdir /data/local/.kali
   sudo mv  /data/local/kali-arm64/*  /data/local/.kali
   sudo rm -rf /data/local/kali-arm64
   ...
   su -c chroot  /data/local/.kali /bin/su -c "locale-gen en_US.UTF-8 2>/dev/null"
    ...
    wget -q https://raw.githubusercontent.com/eirkkk/Droidbox/main/start-kali  > /dev/null 2>&1
    wget -q https://raw.githubusercontent.com/eirkkk/Droidbox/main/kali  > /dev/null 2>&1
    chmod +x start-kail kali
    wget https://kali.download/nethunter-images/current/rootfs/kalifs-arm64-minimal.tar.xz  > /dev/null 2>&1
   ...
   mv $HOME/kali-arm64 .kali 
rm  kalifs-arm64-minimal.tar.xz
